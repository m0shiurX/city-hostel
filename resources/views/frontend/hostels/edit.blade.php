            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.hostels.update", [$hostel->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label class="required" for="user_id">{{ trans('cruds.hostel.fields.user') }}</label>
                            <select class="form-control select2" name="user_id" id="user_id" required>
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $hostel->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                                <div class="invalid-feedback">
                            ...
                            @endif
                            <span class="help-block">{{ trans('cruds.hostel.fields.user_helper') }}</span>
                        </div>
                        <div class="form-group">
