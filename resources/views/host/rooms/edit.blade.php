...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("host.rooms.update", [$room->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('room_info') ? 'is-invalid' : '' }}" type="text" name="room_info" id="room_info" value="{{ old('room_info', $room->room_info) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $room->price) }}" step="0.01">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('placement') ? 'is-invalid' : '' }}" type="text" name="placement" id="placement" value="{{ old('placement', $room->placement) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('facilities') ? 'is-invalid' : '' }}" name="facilities[]" id="facilities" multiple>
                    @foreach($facilities as $id => $facility)
                        <option value="{{ $id }}" {{ (in_array($id, old('facilities', [])) || $room->facilities->contains($id)) ? 'selected' : '' }}>{{ $facility }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Room::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $room->status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tag)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $room->tags->contains($id)) ? 'selected' : '' }}>{{ $tag }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
                    <div class="invalid-feedback">
