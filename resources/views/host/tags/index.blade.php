...
@can('tag_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('host.tags.create') }}">
                    ...
                    @foreach($tags as $key => $tag)
                        <tr data-entry-id="{{ $tag->id }}">
                            ...
                            <td>
                                ...
                                @can('tag_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('host.tags.edit', $tag->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('tag_delete')
                                    <form action="{{ route('host.tags.destroy', $tag->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('tag_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('host.tags.massDestroy') }}",
