...
@can('facility_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('host.facilities.create') }}">
                ...
                <tbody>
                    @foreach($facilities as $key => $facility)
                        <tr data-entry-id="{{ $facility->id }}">
                            ...
                            <td>
                                @can('facility_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('host.facilities.show', $facility->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('facility_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('host.facilities.edit', $facility->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('facility_delete')
                                    <form action="{{ route('host.facilities.destroy', $facility->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('facility_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('host.facilities.massDestroy') }}",
