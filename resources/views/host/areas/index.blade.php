...
@can('area_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('host.areas.create') }}">
                    ...
                    @foreach($areas as $key => $area)
                        <tr data-entry-id="{{ $area->id }}">
                            ...
                            <td>
                                ...
                                @can('area_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('host.areas.edit', $area->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('area_delete')
                                    <form action="{{ route('host.areas.destroy', $area->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('area_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('host.areas.massDestroy') }}",
