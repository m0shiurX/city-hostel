@can('hostel_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('host.hostels.create') }}">
...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-categoryHostels">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($hostels as $key => $hostel)
                        <tr data-entry-id="{{ $hostel->id }}">
                            ...
                            <td>
                                @can('hostel_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('host.hostels.show', $hostel->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('hostel_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('host.hostels.edit', $hostel->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('hostel_delete')
                                    <form action="{{ route('host.hostels.destroy', $hostel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('hostel_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('host.hostels.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-categoryHostels:not(.ajaxTable)').DataTable({ buttons: dtButtons })
